...
class SMOTENCRUS(BaseSampler):
    ...
    _parameter_constraints = {
        "sampling_strategy": [str, dict, callable],
        "random_state": [Hidden, None, int],
        "smote": [None, SMOTE],  # or more generally BaseOverSampler
        "rus": [None, RandomUnderSampler],
        "nc": [None, NeighbourhoodCleaningRule],
        "n_jobs": [Hidden, None, int],
    }
    ...
    def _fit_resample(self, X, y,**params):
        ...
        X_res, y_res = self.nc_.fit_resample(X, y, **params)
        X_res, y_res = self.rus_.fit_resample(X_res, y_res, **params)
...
class iBRFbase(RandomForestClassifier):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        X, y = validate_data(self,
            X=X, y=y,  accept_sparse="csc", dtype=DTYPE
        )
        if sample_weight is not None:
        ...
        if n_more_estimators < 0:
            ...
            print("Make_sampler_estimator, appending trees and samplers!")
            trees = []
            samplers = []
            ...
            )(delayed(_local_parallel_build_trees)(s,t,self,X,y_encoded,sample_weight,i,len(trees),
                    verbose=self.verbose,
            ...
            print("Create pipeline with the fitted samplers and trees!")
...
class iBRF():
    """
    Ref.:
        [1] A. Newaz, M. S. Mohosheu, M. A. A. Noman and T. Jabid, "iBRF: Improved Balanced
            Random Forest Classifier," 2024 35th Conference of Open Innovations Association (FRUCT),
            Tampere, Finland, 2024, pp. 501-508, doi: 10.23919/FRUCT61870.2024.10516372.
        [2] https://github.com/newaz-aa/iBRF/blob/main/iBRF.py
    """
    def __init__(self, config, train_examples, adapt_examples=None):
        ...
        self.avail_labels = train_examples["label"]

        if adapt_examples is not None:
            self.avail_data=torch.cat([self.avail_data, adapt_examples["data"]], dim=0)
            self.avail_label=torch.cat([self.avail_labels, adapt_examples["label"]], dim=0)
        ...
        if self.valid_ratio is not None:
            ...
            self.train_set = {"data": self.avail_data[:train_num], "label": self.avail_labels[:train_num]}
            self.valid_set = {"data": self.avail_data[train_num:], "label": self.avail_labels[train_num:]}
        else:
            # --train-validation no-split (applicable when data size is too small)
            self.train_set = {"data": self.avail_data, "label": self.avail_labels}
            self.valid_set = {"data": self.avail_data, "label": self.avail_labels}

    def train_val(self):
        # You may use out-of-bag score for validation of random forest
        print("Start fitting the random forest")
        X = self.train_set["data"].cpu().numpy()
        y = self.train_set["label"].cpu().numpy().ravel()
