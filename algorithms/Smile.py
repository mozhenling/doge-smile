...
class Smile(BaseTool):
    ...
    This tool is useful for fault detections of rotating machines/bearings, especially when data is skewed, limited, and out-of-distributed.
    ...
    def similar_examples(self, stats, database, database_path=None):
        ...
        assert len(database) >= self.local_top_k, f"tok_k = {self.local_top_k} must be smaller than the current database length ={len(database)}"
